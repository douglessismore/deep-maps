...
export function BottomSheet({ children, onSnapChange, snapTo: snapToProp }: BottomSheetProps) {
  ...
  // Respond to external snapTo prop — always re-snap to ensure sheet position
  // matches the prop, even if the prop value hasn't changed (e.g., navigation
  // resets to 'half' but sheet was dragged away from half position).
  const prevSnapProp = useRef(snapToProp);
  useEffect(() => {
    if (!snapToProp || !isMobile) return;
    const changed = snapToProp !== prevSnapProp.current;
    const drifted = snapToProp !== currentSnapRef.current;
